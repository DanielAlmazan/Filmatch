...
final class DiscoverViewModel {
  ...
  @MainActor private func getCleanVisitedItems(
    ...
    switch media {
    case .movie: result = await movsyRepository.getMovieVisitsByIds(for: ids)
    case .tvSeries: result = await movsyRepository.getTvVisitsByIds(for: ids)
    }
  ...
  @MainActor func discoverItems(
    ...
    if let items = self.items, items.isEmpty {
      ...
      if self.currentPage <= 3 {
        ...
        let latestPageResult = await self.movsyRepository
  ...
  func createVisitedFilterHash(for filter: MediaFilters, at page: Int = 1)
    async
  {
    let result = await movsyRepository.createVisitedFiltersHash(
      for: filter, at: page)
