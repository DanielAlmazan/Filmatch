...
final class SearchUsersResponse: Sendable {
  let page: Int
  let results: [MovsyUserResponse]
  let resultsPerPage: Int
  let totalPages: Int
  let totalResults: Int
  
  init(page: Int, results: [MovsyUserResponse], resultsPerPage: Int, totalPages: Int, totalResults: Int) {
    self.page = page
...
extension SearchUsersResponse: Decodable {
  ...
  convenience init(from decoder: any Decoder) throws {
    ...
    let results = try container.decode([MovsyUserResponse].self, forKey: .results)
