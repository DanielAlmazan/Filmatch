...
struct MovieDetailView: View {
  ...
  init(repository: Repository, filmId: Int) {
    ...
    movieViewModel = MovieDetailViewModel(repository: repository)
  ...
  var body: some View {
    ScrollView {
      if movieViewModel.isMovieLoading {
        ProgressView("Loading...")
      } else if let movie = movieViewModel.movie {
        ...
        VStack(alignment: .leading, spacing: 16) {
          HStack {
            ...
            minutesToHoursAndMinutes(minutes: movie.runtime)
              .font(.footnote)
          }
    ...
    .onAppear {
      movieViewModel.loadMovie(byId: movieId)
    }
    .ignoresSafeArea(edges: .top)
  }

  func minutesToHoursAndMinutes(minutes: Int) -> Text {
...
#Preview{
  ...
  return MovieDetailView(repository: JsonPresetRepository(), filmId: alienFilmId)
}
